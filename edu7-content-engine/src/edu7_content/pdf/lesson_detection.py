...
from .semantic_blocks import classify_block
...
def detect_lesson_start_candidates(reader: Any, start_pdf: int, end_pdf: int) -> List[Dict[str, Any]]:
    ...
    for pdf_page in range(max(1, start_pdf), min(reader.page_count, end_pdf) + 1):
        ...
        if not blocks:
            ...
            if LESSON_RE.search(text or ""):
                candidates.append({
                    "pdfPage": pdf_page,
                    "score": 0.75,
                    "evidence": ["explicit_lesson_heading_text_fallback"],
                })
            continue
        ...
        for block in blocks:
            ...
            if not LESSON_RE.search(text):
                continue
            evidence = ["explicit_lesson_heading"]
...
def reconcile_lesson_range(
    ...
    *,
    ...
    candidates = detect_lesson_start_candidates(reader, toc_start_pdf, end)
