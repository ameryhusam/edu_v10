...
TYPE_PATTERNS = (
    ...
    ("TRUE_FALSE", re.compile(r"\b(?:صح|خطأ|صواب)\b", re.I)),
    ("ORDERING", re.compile(r"\b(?:رتب|رتبي|تصاعديا|تنازليا)\b", re.I)),
...
def group_cross_page_questions(pages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    groups: List[Dict[str, Any]] = []
    previous: Dict[str, Any] | None = None
    for page in sorted(pages, key=lambda p: int(p.get("pdfPage", 0) or 0)):
        for block in extract_question_blocks(page):
            num = block.get("number")
            can_continue = bool(
                previous
                and num is not None
                and previous.get("number") is not None
                and num == previous["number"] + 1
            )
            if previous and can_continue and previous.get("printedPage") != block.get("printedPage"):
                group_id = previous["groupId"]
