...
export function GradesActiveView({ grades, textbooks, selectedPartOrdinal, onSelectPartOrdinal, onSelectGrade, onOpenCreateModal, onOpenBulkModal, onOpenAccreditModal, onOpenWorkspaceModal, statusFilter, onSelectStatus }: GradesActiveViewProps): ReactNode {
  const { t } = useI18n();
  ...
  const gradeBooksForPart = (gradeKey: string) => textbooks.filter((b) => {
    if (b.gradeKey !== gradeKey) return false;
    const part = b.part;
    const inPart = selectedPartOrdinal === 1 ? part === 'PART_1' : part === 'PART_2';
    return inPart && (!statusFilter || b.status === statusFilter);
  });
  const statsByGradeKey = useMemo(() => {
    const map = new Map<string, GradeCardStats>();
    for (const grade of activeGrades) {
      const books = gradeBooksForPart(grade.key);
      map.set(grade.key, {
        total: books.length,
    ...
    return map;
  }, [activeGrades, textbooks, selectedPartOrdinal, statusFilter]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">{filteredGrades.map((grade) => <GradeCardItem key={grade.key} grade={grade} stats={statsByGradeKey.get(grade.key) || { total: 0, published: 0, draft: 0, units: 0, questions: 0 }} activePartName={activePartName} onSelectGrade={onSelectGrade} />)}</div>
    </div>
  );
}
